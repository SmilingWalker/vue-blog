<template>
  <div class="articleContainer">
    <div class="skeleton" v-if="ArticleLoading">
        <el-skeleton class="concise container" v-for="item of 2" :key="'ArticleLoading'+item" active/>
    </div>
    <div  v-else   class="content" >
      <div class="header"  v-if="article.headShow">
        <div class="cover" :style="article.img?`background-image: url(${article.img})`:''"></div>
        <div class="info">
          <h4 class="title">{{ article.title }}</h4>
          <p class="desc">
            <span class="el-icon-s-custom">{{ article.title }}</span>
            <!-- <span icon="el-icon-price-tag">{{ article.labelList }}</span> -->
            <span class="el-icon-view">{{ article.click }}</span>
            <span class="el-icon-alarm-clock">{{ article.createdAt }}</span>
          </p>
        </div>
    ...
    <div class="container desc">
      <div class="skeleton"  v-if="ArticleLoading">
          <el-skeleton  class="concise skeleton container" active/>
      </div>
      <div v-else class="copyright concise">
        <div class="blog-info" v-if="article.copyright_show">
          <p>
            本文标题:
        ...
        <div class="skeleton"  v-if="ArticleLoading">
          <el-skeleton class="concise container" active/>
        </div>
        <template  v-else  > 
            <Comment v-if="article.message_show" class="concise" @submit="submit" />
        </template>
      <div v-if="article.messageShow" class="message" v-infinite-scroll="getMess" :infinite-scroll-distance="20" :infinite-scroll-disabled="loading" infinite-scroll-window="false">
        <MessageCard
          @reply="reply"
          @delete="deleteMess"
          :item="item"
          v-for="(item, index) of MessList"
          :key="item.id"
          class="concise"
          :class="'message-list-item-bg-' + ((index + 1) % 4)"
        />
         <div class="skeleton" v-if="false">
          <el-skeleton v-for="item of 3" :key="'Skeleton'+item" :class="'message-list-item-bg-'+(item+1)%4"  class="concise" avatar active></el-skeleton>
        </div>
...
<script>
import MessageCard from "../message/message-card.vue"
...
export default {
  ...
  setup(){
    ...
    const init = ()=>{
      ...
      blog.QueryArticleById(id).then((result) => {
      ...
      blog.QueryMessageByArticleId(id).then(
        (res)=>{
          while(MessList.length > 0) MessList.pop();
          MessList.push(... res.data.messageList)
