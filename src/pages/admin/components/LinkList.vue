  ...
  <div v-loading="loading">
    ...
    <el-table v-if="list" :data="list" style="width: 100%">
      ...
      <el-table-column label="网站名称">
        <template #default="scope">
          <el-link :href="scope.row.link" type="success">{{
            scope.row.name
          }}</el-link>
        </template>
      </el-table-column>

      <el-table-column label="网站介绍">
        <template #default="scope">
    ...
    <el-dialog :title="title + name + '信息'" v-model="dialogFormVisible">
      <el-form :model="form">
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button @click="updateLink">确 定</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    updateLink() {
      let that = this;
      that
        .$post("/apis/admin/link/" + that.type, that.form)
        .then(function (res) {
          that.$message({
            message: res.message,
            type: "success",
    ...
    setDelete(id) {
      ...
      let that = this;
      that
        .$post("/apis/admin/link/remove?id=" + id)
        .then(function (res) {
          console.log(res);
          that.$message({
            message: res.message,
    ...
    getArticleList() {
      ...
      that.loading = true;
      that
        .$post("/apis/admin/link/list?page=" + that.page)
        .then(function (res) {
          console.log(res);
